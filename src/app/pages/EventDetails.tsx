...
import { useReviewsByEventId } from '../hooks/useReviews';

export const EventDetails = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { event, loading, error } = useEvent(id);
  const { reviews, loading: reviewsLoading } = useReviewsByEventId(id || '');
    ...
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '24px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
        ...
        {event.price !== undefined && (
          <span style={{ fontSize: '20px', color: '#3b82f6', fontWeight: '600', marginLeft: 'auto' }}>
      ...
      <div style={{ backgroundColor: '#f9fafb', padding: '16px', borderRadius: '8px', marginBottom: '24px' }}>
        <p style={{ fontSize: '14px', color: '#6b7280', marginBottom: '8px' }}>📅 Data e Hora</p>
        <p style={{ fontSize: '16px', color: '#1f2937', marginBottom: '16px' }}>
          {new Date(event.date).toLocaleString('pt-BR')}
        </p>
        <p style={{ fontSize: '14px', color: '#6b7280', marginBottom: '8px' }}>📍 Local</p>
        <p style={{ fontSize: '16px', color: '#1f2937' }}>{event.location}</p>
      </div>
      ...
      <div style={{ marginBottom: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          ...
          <Link
            to={`/avaliacao/criar?eventId=${event.id}`}
            style={{
              padding: '8px 16px',
              backgroundColor: '#3b82f6',
              color: '#fff',
        ...
        {reviewsLoading ? (
          ...
          <p style={{ color: '#6b7280' }}>Ainda não há avaliações para este evento.</p>
        ) : (
