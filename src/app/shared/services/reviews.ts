...
export const createReview = async (data: CreateReviewData): Promise<Review> => {
  try {
    ...
    return {
      ...
      communityId: newReview.community_id || undefined,
...
/**
 * Busca avaliações por ID de comunidade
 */
export const getReviewsByCommunityId = async (communityId: string): Promise<Review[]> => {
  try {
    if (!communityId) {
      console.warn('[getReviewsByCommunityId] communityId é undefined ou vazio');
      return [];
    }

    console.log('[getReviewsByCommunityId] Buscando reviews para communityId:', communityId);
    ...
    console.log('[getReviewsByCommunityId] Resultado:', { count: data?.length || 0, communityId, error: error?.message });

    if (error) {
      console.error('Erro ao buscar avaliações da comunidade:', error);
      throw new Error(`Erro ao buscar avaliações da comunidade: ${error.message}`);
    }
    ...
    return (data || []).map((review) => ({
      ...
      serviceId: review.service_id || undefined,
      eventId: review.event_id || undefined,
      communityId: review.community_id || undefined,
      userId: review.user_id,
      userName: review.profiles?.name || review.author_name || 'Usuário',
      userAvatar: review.profiles?.avatar || undefined,
      author: review.profiles?.name || review.author_name || 'Usuário',
